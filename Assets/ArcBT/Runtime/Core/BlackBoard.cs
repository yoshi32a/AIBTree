    ...
    public class BlackBoard
    {
        ...
        public void SetValue<T>(string key, T value)
        {
            ...
            if (isNewKey || isValueChanged)
            {
                ...
                if (IsImportantKey(key))
                {
                    ...
                    BTLogger.Log(LogLevel.Info, LogCategory.BlackBoard, $"🗂️ BlackBoard[{changeType}]: {key} = {displayValue}");
                }
            }
        }
        ...
        public void RemoveValue(string key)
        {
            if (data.Remove(key))
            {
                ...
                BTLogger.Log(LogLevel.Info, LogCategory.BlackBoard, $"🗂️ BlackBoard: Removed '{key}'");
            }
        }
        ...
        public void Clear()
        {
            data.Clear();
            dataTypes.Clear();
            BTLogger.Log(LogLevel.Info, LogCategory.BlackBoard, "🗂️ BlackBoard: Cleared all data");
        }

        /// <summary>デバッグ用：全てのキーと値を表示</summary>
        public void DebugLog()
        {
            BTLogger.Log(LogLevel.Info, LogCategory.BlackBoard, "🗂️ BlackBoard Contents:");
            foreach (var kvp in data)
            {
                BTLogger.Log(LogLevel.Info, LogCategory.BlackBoard, $"  - {kvp.Key}: {kvp.Value} ({dataTypes[kvp.Key].Name})");
            }
        }
